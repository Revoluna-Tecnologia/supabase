...
BEGIN
    ...
    SELECT EXISTS(
        SELECT 1 FROM public.escalistas WHERE email = user_email
    ) INTO escalista_exists;
    
    IF escalista_exists THEN
        SELECT e.id INTO new_escalista_id
...
BEGIN
    ...
    FROM auth.users u
    LEFT JOIN public.escalistas e ON e.email = u.email
    WHERE u.invited_at IS NOT NULL OR e.id IS NOT NULL
