-- Corrigir campos de auditoria na função gerar_vagas_recorrentes
CREATE OR REPLACE FUNCTION public.gerar_vagas_recorrentes(
  p_recorrencia_id uuid,
  p_vaga_base_id uuid,
  p_medico_id uuid DEFAULT NULL,
  p_created_by uuid DEFAULT NULL  -- NOVO: Adicionar parâmetro para quem está criando
)
RETURNS void
LANGUAGE plpgsql
AS $$
    ...
    IF array_position(dias, extract(dow from dia)::integer) IS NOT NULL THEN
      ...
      IF p_medico_id IS NOT NULL THEN
        INSERT INTO public.candidaturas (
          ...
          p_medico_id, nova_vaga_id, 'APROVADO', now_brasil, now_brasil, audit_user, vaga_base.vagas_valor
        );
      END IF;
