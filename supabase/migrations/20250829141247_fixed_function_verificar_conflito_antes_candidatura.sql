  ...
  BEGIN
    ...
    -- Verificar conflitos de horário (CORRIGIDO PARA TURNOS NOTURNOS)
    SELECT 
        EXISTS (
            SELECT 1
            FROM candidaturas c
            JOIN vagas v ON c.vagas_id = v.vagas_id
            WHERE c.medico_id = medico_userid
...
BEGIN
    ...
    SELECT NEW.medico_id, v.vagas_data, v.vagas_horainicio, v.vagas_horafim
