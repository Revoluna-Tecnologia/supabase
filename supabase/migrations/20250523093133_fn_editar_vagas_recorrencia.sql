-- Função para editar em massa todas as vagas de uma recorrência
...
BEGIN
  FOR vaga IN SELECT * FROM public.vagas WHERE recorrencia_id = p_recorrencia_id LOOP
    UPDATE public.vagas SET
      ...
      vagas_horainicio = COALESCE((p_update->>'vagas_horainicio'), vagas_horainicio),
      vagas_horafim = COALESCE((p_update->>'vagas_horafim'), vagas_horafim),
