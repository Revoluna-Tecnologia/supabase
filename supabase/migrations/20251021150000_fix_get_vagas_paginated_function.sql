...
RETURN QUERY WITH vagas_agrupadas AS (
  SELECT v.vagas_id,
    ...
    (array_agg(v.id)) [1] AS escalista_id,
...
COMMENT ON FUNCTION get_vagas_paginated IS 'Busca vagas agrupadas por vagas_id com filtros opcionais usando arrays em snake_case. Filtros disponíveis: hospital_ids[], specialty_ids[], sector_ids[], period_ids[], type_ids[], group_ids[], doctor_ids[], application_status_filter[PENDENTE,APROVADO,REPROVADO], job_status_filter[aberta,fechada,cancelada,anunciada], grade_ids[], além de filtros de data, valor e texto. Parâmetros de ordenação: order_by[vagas_createdate,vagas_data,vagas_valor,hospital_nome,setor_nome,especialidade_nome,vagas_periodo_nome,vagas_status,total_candidaturas], order_direction[ASC,DESC]. Todas as candidaturas são compactadas em array por vaga, resultando em menos páginas totais';