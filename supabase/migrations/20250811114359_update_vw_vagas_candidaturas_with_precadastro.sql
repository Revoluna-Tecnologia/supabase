-- Atualizar view vw_vagas_candidaturas para incluir médicos pré-cadastrados
-- usando estratégia one-for-all

DROP VIEW IF EXISTS vw_vagas_candidaturas;

CREATE VIEW vw_vagas_candidaturas AS
SELECT 
    row_number() OVER (ORDER BY combined_data.vagas_id, combined_data.effective_medico_id, combined_data.candidaturas_id) AS idx,
...
FROM (
    SELECT DISTINCT 
        ...
        -- ID efetivo do médico (real para confirmados, precadastro para pré-cadastrados)
        CASE 
            WHEN c.medico_id = '9cd29712-91b5-492f-86ff-41e38c7b03d5' AND c.medico_precadastro_id IS NOT NULL 
            THEN c.medico_precadastro_id
            ELSE vm.medico_id
        END AS effective_medico_id,
        -- Dados do médico unificados (confirmado ou pré-cadastrado)
        ...
        COALESCE(m.medico_email, mp.medico_email) AS medico_email,
        COALESCE(m.medico_telefone, mp.medico_telefone) AS medico_telefone,
    ...
    -- União de médicos confirmados e pré-cadastrados nas candidaturas
    LEFT JOIN (
        -- Médicos confirmados
        SELECT 
        ...
        WHERE candidaturas.medico_id IS NOT NULL 
          AND candidaturas.medico_id != '9cd29712-91b5-492f-86ff-41e38c7b03d5'
        
        UNION
        
        -- Médicos pré-cadastrados (usar ID pré-cadastro como ID efetivo)
        SELECT 
            candidaturas.vagas_id,
            candidaturas.medico_precadastro_id AS medico_id
        FROM candidaturas
        WHERE candidaturas.medico_id = '9cd29712-91b5-492f-86ff-41e38c7b03d5' 
          AND candidaturas.medico_precadastro_id IS NOT NULL
        
        UNION
        
        -- Vagas salvas por médicos confirmados
        SELECT 
    ...
    -- Join com candidaturas (considerando estratégia one-for-all)
    LEFT JOIN candidaturas c ON c.vagas_id = v.vagas_id 
        AND (
            (c.medico_id = vm.medico_id AND c.medico_id != '9cd29712-91b5-492f-86ff-41e38c7b03d5')
            OR 
            (c.medico_id = '9cd29712-91b5-492f-86ff-41e38c7b03d5' AND c.medico_precadastro_id = vm.medico_id)
        )
    
    -- Join com dados dos médicos confirmados
    LEFT JOIN medicos m ON m.id = vm.medico_id
    
    -- Join com dados dos médicos pré-cadastrados
    LEFT JOIN medicos_precadastro mp ON mp.id = vm.medico_id
    
    -- Vagas salvas por médicos confirmados
    LEFT JOIN vagas_salvas vs ON vs.vagas_id = v.vagas_id AND vs.medico_id = vm.medico_id
    
    -- Vagas salvas por médicos pré-cadastrados (se implementado no futuro)
    LEFT JOIN vagas_salvas vsp ON vsp.vagas_id = v.vagas_id 
        AND vsp.medico_id = '9cd29712-91b5-492f-86ff-41e38c7b03d5'
        -- Assumindo que no futuro podemos ter vagas_salvas.medico_precadastro_id
    
    -- Check-in/checkout para médicos confirmados
    LEFT JOIN checkin_checkout cc ON cc.vagas_id = v.vagas_id AND cc.medico_id = vm.medico_id
    
    -- Check-in/checkout para médicos pré-cadastrados (usando ID efetivo)
    LEFT JOIN checkin_checkout ccp ON ccp.vagas_id = v.vagas_id 
        AND ccp.medico_id = (
            CASE 
                WHEN c.medico_id = '9cd29712-91b5-492f-86ff-41e38c7b03d5' 
                THEN c.medico_precadastro_id 
                ELSE vm.medico_id 
            END
        )
