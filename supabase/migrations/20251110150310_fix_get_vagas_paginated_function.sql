-- Migration: Fix get_vagas_paginated function with correct column names
-- Date: 2025-10-14

-- =====================================================
-- Correção da função get_vagas_paginated
-- =====================================================
...
-- Comentário atualizado da função  
COMMENT ON FUNCTION get_vagas_paginated IS 'Busca vagas agrupadas com suas candidaturas usando filtros opcionais. Filtros disponíveis: hospital_ids[], specialty_ids[], sector_ids[], period_ids[], type_ids[], group_ids[], doctor_ids[], application_status_filter[PENDENTE,APROVADO,REPROVADO], job_status_filter[aberta,fechada,cancelada,anunciada], grade_ids[], além de filtros de data, valor e texto. Parâmetros de ordenação: order_by[vagas_createdate,vagas_data,vagas_valor,hospital_nome,setor_nome,especialidade_nome,vagas_periodo_nome,vagas_status,total_candidaturas], order_direction[ASC,DESC]. Retorna vagas agrupadas com array de candidaturas associadas.';