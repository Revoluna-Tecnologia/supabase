...
-- Recriar a view com lógica corrigida para vaga_salva
...
SELECT row_number() OVER (ORDER BY v.vagas_id, c.candidaturas_id) AS idx,
    ...
    v.recorrencia_id,
    CASE 
        WHEN EXISTS (SELECT 1 FROM vagas_salvas vs WHERE vs.vagas_id = v.vagas_id AND vs.medico_id = COALESCE(m.id, auth.uid())) THEN true
        ELSE false 
    END AS vaga_salva
FROM vagas v
